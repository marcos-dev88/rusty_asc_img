...
pub fn gen_ascii_art(path: &str, width: u32, heigth: u32) -> String {
    let img = image::open(path).unwrap();

    let mut img_buff: image::ImageBuffer<image::Rgba<u8>, Vec<u8>> = image::imageops::resize(&img, width, heigth, image::imageops::FilterType::Lanczos3);

    if img.width() >= width || img.height() >= heigth {
        img_buff = image::imageops::resize(&img, width, heigth, image::imageops::FilterType::Nearest);
    }
